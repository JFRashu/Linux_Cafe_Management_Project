...
create_Bill() {
    ...
    local total_bill=$(echo "SELECT 
                ...
                WHERE 
                    DATE_FORMAT(o.\`time\`, '%Y-%m') = '$current_month'" | "$MYSQL_EXECUTABLE" -u "$DB_USER" "$DB_NAME" -N)

    local total_orders=$(echo "SELECT COUNT(*) FROM \`order\` WHERE DATE_FORMAT(\`Time\`, '%Y-%m') = '$current_month'" | "$MYSQL_EXECUTABLE" -u "$DB_USER" "$DB_NAME" -N)
        ...
        case $choice in 
            1)
                ...
                JOIN 
                    \`order\` o ON inc.\`Order_code\` = o.\`Order_code\`
                WHERE 
                    DATE_FORMAT(o.\`time\`, '%Y-%m') = '$current_month'
                GROUP BY 
